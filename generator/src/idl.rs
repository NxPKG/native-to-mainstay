...
//
// Remaining accounts will be commented out in the dummy Mainstay program.
...
use mainstay_syn::idl::{self, Idl, IdlType};
...
pub struct ProgramAndIdlGenerator<'a> {
    ...
    all_content: &'a str,
    /// Mainstay lib.rs path
    mainstay_path: PathBuf,
    /// All content that will be written to program/lib.rs
    mainstay_content: RefCell<String>,
...
pub struct IdlGenerator<'a> {
    pub idl_path: &'a Path,
    /// Whether to keep generated mainstay program folder after IDL creation
    pub keep_dummy_program: bool,
}

impl<'a> ProgramAndIdlGenerator<'a> {
    pub fn new_program(generator: &'a Generator) -> Self {
        let mainstay_path = generator
            .generated_project_path
            .join(dirname::PROGRAM)
            .join(filename::LIB);

        let mainstay_content = PROGRAM_PRETEXT.replace(
            "<ProgramName>",
        ...
        Self {
            ...
            mainstay_path,
            mainstay_content: RefCell::new(mainstay_content),
    ...
    pub fn new_idl(
        ...
        let (mainstay_path, mainstay_content, program_generator) = match maybe_dummy_program_path {
            Some(path) => {
    ...
    pub fn run(self) -> GeneratorResult {
        ...
        if self.program_generator.is_some() {
            info(format!(
                "Creating Mainstay program for {}...",
                self.program_info.name
            ));

            // Create Mainstay dir(s) recursively
            let mainstay_dir = &self.mainstay_path.parent().unwrap();
            if !Path::new(mainstay_dir).is_dir() {
                fs::create_dir_all(mainstay_dir)?;
            }

            // Format the code
            rustfmt(&self.mainstay_path)?;
    ...
    fn create_instructions(&self) -> GeneratorResult {
        ...
        let mut mainstay_content = self.mainstay_content.borrow_mut();
        ...
        'line_loop: for line in content.lines() {
            ...
            if open_count > 0 {
                ...
                mainstay_content.push_str(&line);

                if line.len() > 1 {
                    mainstay_content.push('\n');
                }
            }
        ...
        mainstay_content.pop(); // Remove ',' at the end
        mainstay_content.push_str("}\n\n"); // space for Contexts

        // Add contexts
        mainstay_content.push_str(&contexts);
    ...
    fn create_accounts_from_content(&self, content: String) -> GeneratorResult {
        ...
        for account in &accounts {
            ...
            mainstay_content.push_str(&format!("{account}\n"));
        }
        ...
        for ty in types {
            ...
            let ty = format!("#[derive(MainstaySerialize, MainstayDeserialize)]\n{ty}\n");
            mainstay_content.push_str(&ty);
        }
        ...
        loop {
            ...
            for type_name in type_names.borrow().iter() {
                let indices = get_item_indices(mainstay_content.as_str(), type_name);
                if indices.len() == 1 {
                    debug(format!("Type '{type_name}' is only used once. Removing..."));

                    let item = get_local_type(type_name, mainstay_content.as_str()).unwrap();
                    let mainstay_serde_len =
                        "#[derive(MainstaySerialize, MainstayDeserialize)]".len() + 1;
                    let start_index = match get_item_type_from_full_item(&item) {
                        "struct" => indices[0].1 - (("pub struct".len() + 1) + mainstay_serde_len),
                        "enum" => indices[0].1 - (("pub enum".len() + 1) + mainstay_serde_len),
                        _ => unreachable!(),
                    };
                    mainstay_content.replace_range(
                        start_index..start_index + item.len() + mainstay_serde_len,
                        "",
                    );
                    used_once_count += 1;
                }
    ...
    fn create_errors(&self) -> GeneratorResult {
        ...
        if let Some(error_index) = content.find(&format!("pub enum")) {
            ...
            let mut mainstay_content = self.mainstay_content.borrow_mut();
            mainstay_content.push_str(&format!("#[error_code]{error}"));
        }

        Ok(())
    }

    fn write_mainstay_dummy(&self) -> GeneratorResult {
        let mut mainstay_content = self.mainstay_content.borrow_mut();

        for replecable_type in self.replecable_types.borrow().iter() {
            (*mainstay_content) = (*mainstay_content)
                .replace(
        ...
        fs::write(&self.mainstay_path, mainstay_content.as_bytes())?;
    ...
    fn create_idl(&self) -> GeneratorResult {
        ...
        for ix in &idl.instructions {
            for arg in &ix.args {
                ...
                if let IdlType::Defined(name) = arg_type {
                    ...
                    let mut mainstay_content = self.mainstay_content.borrow_mut();
                    if let None = get_local_type(name, mainstay_content.as_str()) {
                        // Defined type doesn't exist in mainstay file
                        // Try to find it from all content
                        match get_local_type(name, self.all_content) {
                            Some(defined_type) => {
                                info!("{}", format!("Adding missing type '{name}'").purple());

                                // Add the type to mainstay content
                                mainstay_content.push_str(&format!("\n#[derive(MainstaySerialize, MainstayDeserialize)]\n{defined_type}\n"));
                                new_idl = true;
                            }
        ...
        // Delete mainstay dummy if configured
        if !self.idl_generator().keep_dummy_program {
            // Dangerous O_o
            // fs::remove_dir_all(self.mainstay_path.parent().unwrap())?;
            fs::remove_file(&self.mainstay_path)?;
            fs::remove_dir(self.mainstay_path.parent().unwrap())?;
        }
